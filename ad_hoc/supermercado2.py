        ...
        for i in range(n):
            ...
            for k in range(n):
                if corredor_supermercado[k] == lista_compra[j]:
                    ...
                    elif preco[k] > maior or len(usados) == 1:
                        montante += preco[k]
                        maior = preco[k]
                        usados.append(lista_compra[j])
                    elif j == len(lista_compra) - 1 and preco[k] < maior:
                        montante -= maior
                        montante += preco[k]
                        maior = preco[k]
                        usados.append(lista_compra[j])
                    elif (j < len(lista_compra) - 1 and lista_compra[j + 1] == lista_compra[j] and
                          preco[k] < maior):
                        montante += preco[k]
                        maior = preco[k]
                        usados.append(lista_compra[j])
                    ...
                    if j == len(lista_compra):
                        j -= 1
                    if lista_compra.count(usados[-1]) > usados.count(usados[-1]):
                        j -= 1
                    if cont >= len(lista_compra) and j < 1:
                        j += 1
                        cont = 0
                        if j == len(lista_compra):
                            j -= 1
                    elif cont >= len(lista_compra) and j >= 1:
                        j += 1
                        cont = 0
                        if j == len(lista_compra):
                            j -= 1
                if len(usados) >= len(lista_compra):
                    iterar_usados = iter(usados)
                    resultado = all(x in iterar_usados for x in lista_compra)
                    if resultado:
