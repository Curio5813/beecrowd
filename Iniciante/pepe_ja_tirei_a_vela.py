def main():
    ...
    funcionar igualmente ao projeto inicial. Colocaram um sensor de pressão embaixo
    da vela do candelabro, de modo que a retirada da vela ativa o sistema. Este
    sistema emite um relatório de log por cada vez que a porta abriu ou fechou,
    mas o log está bem confuso. A cada registro, três números inteiros são cadastrados,
    sendo a hora e o minuto que o evento ocorreu e um valor que representa se a
    porta abriu ou fechou naquele momento. Pepe pede a sua ajuda para converter
    os dados do log em dados mais legíveis para ele.

    Escreva um programa que, dado um registro de log, este seja convertido em textos
    mais legíveis.
    ...
    for k in range(t):
        ...
        if log[2] == 0:
            log[2] = 'A porta fechou!'
        elif log[2] == 1:
            log[2] = 'A porta abriu!'
        if log[0] < 10 and log[1] < 10:
            print(f'0{log[0]}:0{log[1]} - {log[2]}')
        elif log[0] < 10 and log[1] >= 10:
            print(f'0{log[0]}:{log[1]} - {log[2]}')
        elif log[0] >= 10 and log[1] < 10:
            print(f'{log[0]}:0{log[1]} - {log[2]}')
        else:
