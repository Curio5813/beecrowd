def main():
    ...
    linha contendo um horário H (5:00 ≤ H ≤ 9:00) que Bino acordou. A entrada termina
    com final de arquivo (EOF).

    Saída
    Para cada caso de teste, imprima "Atraso maximo: X" (sem aspas), X indica o atraso
    maximo (em minutos) de Bino no encontro com Cino.
    """
    while True:
        try:
            h = input().split(':')
            ...
            if h[0] == 7 and h[1] == 0:
                print('Atraso maximo: 0')
            if h[0] == 7 and h[1] > 0:
                print(f'Atraso maximo: {60 - 60 + h[1]}')
            if h[0] == 8:
                print(f'Atraso maximo: {60 + h[1]}')
            if h[0] == 9:
                print(f'Atraso maximo: {120 + h[1]}')
